...
const GoogleActivity = ({ db, colors, filterBar = true }) => {
  ...
  const render = async () => {
    if (db && db.activity) {
      ...
      activities = activities.filter(({app}) => app.includes('google: '));
      if (filteredActivities.size > 0) activities = activities.filter(({app}) => filteredActivities.has(app.replace('google: ', '')));
      ...
      activities = activities.map(({app, timestamps}) => {
        ...
        return { app: app.replace('google: ', ''), record };
      })
      const months = [];
  ...
  return (
    ...
    <div>
      <InfoCard
        ...
        description='Your monthly activity for activities on Google products, you may select up to 7 products to compare them, the selection defaults to the top 7 products you have interacted with'
        benefits='Google stores your activity with timestamps attached, this can help you understand what you have done on the platform and when'
        sources={['Google MyActivity']}
      />
      ...
      {filterBar && <div className='filter-bar'>
        ...
        <select id='activity-filter' defaultValue='all' disabled={filteredActivities.size === 7}
          ...
          { db.activity.filter(({app}) => app.includes('google: ')).map(activity => ({...activity, app: activity.app.replace('google: ', '')})).sort((a, b) => a.app.toLowerCase().localeCompare(b.app.toLowerCase())).map(({app}) => <option value={app}>{app}</option>) }
        </select>
